package com.test.bootanimation.sevenencode.lzma;

import java.io.IOException;

import com.test.bootanimation.sevenencode.ICodeProgress;
import com.test.bootanimation.sevenencode.lz.BinTree;
import com.test.bootanimation.sevenencode.rangecoder.BitTreeEncoder;
import com.test.bootanimation.sevenencode.rangecoder.RangeCoderEncoder;
...
public class Encoder
{
	...
	class LiteralEncoder
	{
		class Encoder2
		{
			...
			public void Init() { RangeCoderEncoder.InitBitModels(m_Encoders); }
	...
	{
		return RangeCoderEncoder.GetPrice0(_isRepG0[state]) +
				RangeCoderEncoder.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
	}
	...
	int GetOptimum(int position) throws IOException
	{
		...
		_optimum[1].Price = RangeCoderEncoder.GetPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]) +
		...
		while (true)
		{
			...
			int curAnd1Price = curPrice +
					RangeCoderEncoder.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) +
			...
			if (!nextIsChar && matchByte != currentByte)
			{
				...
				if (lenTest2 >= 2)
				{
					...
					int nextRepMatchPrice = curAnd1Price +
						RangeCoderEncoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
						RangeCoderEncoder.GetPrice1(_isRep[state2]);
					{
				...
				if (lenTest < numAvailableBytesFull)
				{
					...
					if (lenTest2 >= 2)
					{
						...
						int curAndLenCharPrice =
								...
								RangeCoderEncoder.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
